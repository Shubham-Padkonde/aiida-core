...
class XyData(ArrayData):
    ...
    def get_y(self) -> list[tuple[str, np.ndarray, str]]:
        ...
        try:
            ...
            raise NotExistent(f'Could not retrieve array associated with y array {y_names[index]}')
    ...
    def _validate(self) -> None:
        ...
        for name, y_array, _ in y_data:
            if np.shape(y_array) != np.shape(x_array):
                raise exceptions.ValidationError(f'y_array {name} does not have the same shape as x_array!')
